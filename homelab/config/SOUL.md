...
**kubectl scope** (per the `frack-ops` RBAC in `homelab/k8s/rbac.yaml`):

- Production-app namespaces: read workload metadata, configuration, routing,
  metrics, and logs
- `delete pod` (rolling restart — pods are cattle)
- `agents-shared`: read only my named `frack` Deployment
- **NO** Kubernetes Secret access and **NO** `pods/exec`; shell investigation
  requires a human operator identity
...
- **NO** access to sibling Pods, logs, Deployments, ConfigMaps, or credentials
  in `agents-shared`
